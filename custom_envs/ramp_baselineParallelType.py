...
class SumoRampEnv(Env):
    ...
    def getobservedVehicles(self, vehicle_ids):
        ...
        self.rl_car_lane = traci.vehicle.getLaneID(self.bl_car_id)
        self.rl_car_pos = traci.vehicle.getPosition(self.bl_car_id)
        self.rl_car_distance = traci.vehicle.getDistance(self.bl_car_id)
    ...
    def getLeadFollowercarBMerge(self, vehicle_ids):
        ...
        self.rl_car_lane = traci.vehicle.getLaneID(self.bl_car_id)
        self.rl_car_pos = traci.vehicle.getPosition(self.bl_car_id)
        self.rl_car_distance = traci.vehicle.getDrivingDistance(self.bl_car_id, "e35", 0.0)
    ...
    def getObservations(self):
        ...
        if vehicle_ids:
            ...
            state_speed[-1] = traci.vehicle.getSpeed(self.bl_car_id) / self.maxSpeed
            state_position_x[-1] = traci.vehicle.getPosition(self.bl_car_id)[0]
            state_position_y[-1] = traci.vehicle.getPosition(self.bl_car_id)[1]
    ...
    def step(self, action):
        ...
        rl_car_acc_duration = 0.01
        rl_car_acc = 3
        self.bl_car_id = self.addBaselineVehicle()
        currentSpeed = traci.vehicle.getSpeed(self.bl_car_id)
        ...
        traci.vehicle.slowDown(self.bl_car_id, next_vel, rl_car_acc_duration)  # adjust acceleration of ego vehicle

        # traci.vehicle.setAcceleration(self.bl_car_id, rl_car_acc, rl_car_acc_duration)
    ...
    def calc_reward(self):
        ...
        rlAcceleration = traci.vehicle.getAcceleration(self.bl_car_id)
        rlSpeed = traci.vehicle.getSpeed(self.bl_car_id)
        rlLane = traci.vehicle.getRoadID(self.bl_car_id)
        if rlAcceleration > -200:  # avoid the max negative valye
            # if rlAcceleration<0:
            #     # print(rlAcceleration, 'rlAcceleration')

            maxAcc = traci.vehicle.getAccel(self.bl_car_id)
            rj = +1 * abs(self.alphaJ) * rlAcceleration / maxAcc
            # rs = +1 * abs(self.alphaJ) * rlSpeed / traci.vehicle.getMaxSpeed(self.bl_car_id)
        else:
            rj = 0
        ...
        if rlLane == 'e42':  # before merge
            ...
            rearvehicle = traci.vehicle.getFollower(self.bl_car_id)
            leadvehicle = traci.vehicle.getLeader(self.bl_car_id)
            ...
            steeringAngle = traci.vehicle.getAngle(self.bl_car_id)
    ...
    def addRLVehicle(self):
        ...
        if self.bl_car_id not in vehicle_ids:
            self.bl_car_id = "rl_car_" + str(np.random.randint(1e10))  # random generate a rl_car

            traci.vehicle.add(self.bl_car_id, routeID="route_ramp")  # vehicle add
            traci.vehicle.setColor(self.bl_car_id, color=(255, 0, 0, 255))  # change vehicle color to red
    ...
    def addBaselineVehicle(self):
        ...
        if self.bl_car_id not in vehicle_ids:
            ...
            traci.vehicle.add(self.bl_car_id, routeID="route_ramp", typeID="idmAlternative", departSpeed = "random")  # vehicle add
            # traci.vehicle.setColor(self.bl_car_id, color=(0 , 0, 255, 255))  # change vehicle color to blue
            # # print('type_id ', traci.vehicle.getTypeID(self.bl_car_id))
            traci.vehicle.setSpeedMode(self.bl_car_id, 32)  # change speed mode to 55 which is
            self.previoussteeringAngle = 0
            self.checkSuccessOnce = 0
            self.stepRltime = 0
