...
class BsmMerge(SumoRamp):
    ...
    def step(self, action):
        ...
        #print(action, traci.vehicle.getAcceleration(self.rl_car_id),'action , traci acc')
        ...
        info = {
            ...
            'acc_reward' : self.average_changeAcc,
        }
...
class BsmMergeAllRewards(BsmMerge):
    ...
    def getVelocityReward(self): # acceleartion penality and reward
        reward = traci.vehicle.getSpeed(self.rl_car_id)/ traci.vehicle.getMaxSpeed(self.rl_car_id)
    ...
    def getAccJerkReward(self):
        ...
        if rlAcceleration > -200:  # avoid the max negative valye
            ...
            self.average_changeAcc = abs(rlAcceleration - self.oldAcc) / (self.deltaT)
...
#        print(leaddistance, ' lead distance', rearDistance,'rear Distance')

        rd = 2 - (abs(leaddistance) / self.maxDistance) ** self.alphaDistancef - (abs(
                rearDistance) / self.maxDistance) ** self.alphaDistancer
        rj = self.getAccJerkReward()

        rVelocity = self.getVelocityReward()

        reward = rj + rd* self.alphaD  + rVelocity

        #print('rd', rd , 'rd *  alpa ' , rd*self.alphaD)
        #print( rj , 'rj',rAccel,'r Accel')
        #print(reward)

        #print('accelReward',rAccel)
        return reward

    def getAfterMergeReward(self):  # reward after the vehicle has merged
        rj = self.getAccJerkReward()  # we still get the jerk reward
        lead_acc_1 = lead_acc_2 = follower_acc_1 = follower_acc_2 = 0
        lead_velocity_1 = lead_velocity_2 = follower_velocity_1 = follower_velocity_2 = follower_velocity_3 = follower_velocity_4  = 0     
        leadvehicle = traci.vehicle.getLeader(self.rl_car_id)
        vehicle_ids = self.getVehicleIds()
        v_velocity = []
        self.velocity_reward = 0 
        for vehicle in vehicle_ids:
            if traci.vehicle.getLaneID(vehicle).split('_')[-1] == "0":
                norm_v = traci.vehicle.getSpeed(vehicle)/traci.vehicle.getMaxSpeed(vehicle)
                v_velocity.append(traci.vehicle.getSpeed(vehicle)/traci.vehicle.getMaxSpeed(vehicle))
        ...
        #     lead_velocity_1  = traci.vehicle.getSpeed(leadvehicle[0])/ traci.vehicle.getMaxSpeed(leadvehicle[0])
        #
        #     if leadvehicle2 != None:
        #         lead_acc_2 = traci.vehicle.getAcceleration(leadvehicle2[0]) / traci.vehicle.getAccel(self.rl_car_id)
        #
        #         lead_velocity_2  = traci.vehicle.getSpeed(leadvehicle2[0])/ traci.vehicle.getMaxSpeed(leadvehicle2[0])
        # followervehicle = traci.vehicle.getLeader(self.rl_car_id)
        # if  followervehicle != None:
        #     followervehicle2 = traci.vehicle.getFollower(followervehicle[0])
        #     follower_acc_1 = traci.vehicle.getAcceleration(followervehicle[0]) / traci.vehicle.getAccel(self.rl_car_id)
        #
        #     follower_velocity_1  = traci.vehicle.getSpeed(followervehicle[0])/ traci.vehicle.getMaxSpeed(followervehicle[0])
            # if  followervehicle2 != None:
            #
            #     #followervehicle3 = traci.vehicle.getFollower(followervehicle2[0])
            #     follower_acc_2 = traci.vehicle.getAcceleration(followervehicle2[0]) / traci.vehicle.getAccel(self.rl_car_id)
            #     follower_velocity_2  = traci.vehicle.getSpeed(followervehicle2[0])/ traci.vehicle.getMaxSpeed(followervehicle2[0])
            # #    if  followervehicle3 != None:
            #
            #        followervehicle4 = traci.vehicle.getFollower(followervehicle3[0])
            #        follower_acc_3 = traci.vehicle.getAcceleration(followervehicle3[0]) / traci.vehicle.getAccel(self.rl_car_id)
            #        follower_velocity_3  = traci.vehicle.getSpeed(followervehicle3[0])/ traci.vehicle.getMaxSpeed(followervehicle3[0])
            #        if  followervehicle4 != None:
            #
            #            #followervehicle4 = traci.vehicle.getFollower(followervehicle4[0])
            #            follower_acc_4 = traci.vehicle.getAcceleration(followervehicle4[0]) / traci.vehicle.getAccel(self.rl_car_id)
            #            follower_velocity_4  = traci.vehicle.getSpeed(followervehicle4[0])/ traci.vehicle.getMaxSpeed(followervehicle4[0])
            #            print(follower_velocity_4, 'follower 4 speed ')


        # rVel = self.getVelocityReward()
        reward = np.mean(v_velocity)
        self.velocity_reward = reward * traci.vehicle.getMaxSpeed(self.rl_car_id)
        #rVel +  lead_velocity_1 + lead_velocity_2  + follower_velocity_1 + follower_velocity_2 + follower_velocity_3 + follower_velocity_4
        #lead_acc_1 + lead_acc_2 + follower_acc_1 + follower_acc_2  
        #print('accelReward',rAccel)
    ...
    def merge_timeReward(self):
        self.stepRltime = traci.simulation.getTime() # traci.simulation.getDeltaT()   # 
    ...
    def getrewards(self):
        ...
        elif traci.simulation.getTime() ==self.maxEpisodeLength :
            reward = -1 * abs(self.rSuccess)
        ...
        # print('reward before time', reward)
