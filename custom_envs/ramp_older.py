...
class SumoRampEnv(Env):
    ...
    def step(self, action):
        ...
        next_vel = max([rl_car_acc_duration + rl_car_acc * rl_car_acc_duration, 0])

        traci.vehicle.slowDown(self.rl_car_id, next_vel, rl_car_acc_duration)
    ...
    def sumo_reset(self):
        ...
        if self.virtual_display is not None:
            ...
            self.disp = SmartDisplay( visible = 1, size=self.virtual_display)
            self.disp.start()
