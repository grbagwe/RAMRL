...
class SumoRampEnv(Env):
    ...
    def __init__(self, sumoConfPath = None ):
        ...
        self.maxEpisodeLength  = 100 #3600
        ...
        # }
        self.observation_space = Box(low = -5000, high = 5000, shape = (21,))#  gym.spaces.Dict(spaces)
        # self.observation_space = Box(low=)
        self.run =0


        self.sumoConfigFile ='/home/gauravb/Documents/MichiganTech/Programming/CustomRampTraining/custom_envs/sumo_Config/ramp_1.sumocfg'# sumoConfPath
        import traci as traci
        import sumolib
    ...
    def reset(self):
        ...
        self.episodeLength = self.maxEpisodeLength
        self.info = {}
        obs = self.getObservations().astype(np.float32)
        # print('obs', obs.dtype)
    ...
    def getObservations(self):
        ...
        state = np.concatenate((state_speed, state_position_x, state_position_y))
        self.obs_shape = np.float32(len(state))
        return state
    ...
    def step(self, action):
        ...
        # print(np.float32(state).dtype, state)
        return state.astype(float), reward.astype(float), self.done, self.info
    ...
    def saveImageFrame(self):
        ...
        imagePath = os.path.join(self.imagePath, 'frame_{}_{}.png'.format(self.label,self.lastSavedFrame))
