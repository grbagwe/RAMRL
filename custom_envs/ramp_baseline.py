...
class SumoRampEnv(Env):
    ...
    def step(self, action):
        ...
        self.stepRltime += 1
        if traci.vehicle.getRoadID(self.bl_car_id) == "e23":
    ...
    def addBaselineVehicle(self):
        ...
        if self.bl_car_id not in vehicle_ids:
            ...
            traci.vehicle.add(self.bl_car_id, routeID="route_ramp", typeID="IDMPassenger",
                              departSpeed="random")  # vehicle add
            traci.vehicle.setColor(self.bl_car_id, color=(255, 0, 0, 255))  # change vehicle color to blue
            # # print('type_id ', traci.vehicle.getTypeID(self.bl_car_id))
            traci.vehicle.setSpeedMode(self.bl_car_id, 31)  # change speed mode to 55 which is
            self.previoussteeringAngle = 0
            self.checkSuccessOnce = 0
            self.stepRltime = 0
            self.time2merge = 0
