...
from custom_envs.MultiMerge import Image_Noise_No_BSM as MultiMerge
...
if __name__ == '__main__':
    run = wandb.init(
        ...
        name=f"Image_Noise+NoBSM_{args.config}",
        dir=dir,
        config=config,
    ...
    for i_games in range(n_games):
        ...
        avg_collisionRate.append(num_collisions)
        avg_mergeTime.append(mergeTime)
        avg_velocity.append(np.mean(velocity_reward))
        avg_accJerk.append(np.mean(acc_reward))

    print(f"average score {np.mean(avg_score)} num_collisions : {np.mean(avg_collisionRate)} , mergetime : {np.mean(avg_mergeTime)} , acc_rewards :{np.mean(avg_accJerk)} averga velocity: {np.mean(avg_velocity)} ")
