...
parser.add_argument("--noise_sigma",default=0.1 ,  help="Image noise sigma valu")
...
if __name__ == '__main__':
    run = wandb.init(
        project="Robust-OnRampMerging2",
        name=f"BSM+NoImage_{args.config}",
        dir=dir,
        config=config,
    ...
    model = PPO(config["policy_type"], env,
            verbose=3,policy_kwargs=policy_kwargs,
                gamma=0.99,
                n_steps=512,
                ent_coef=0.0905168,
                learning_rate=0.005,
    ...
    model.save(f"{dir}/models/{run.id}/vecstats/")
